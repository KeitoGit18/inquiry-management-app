...
- 名前100文字、メールアドレス254文字、問い合わせ内容5,000文字まで入力可能
- 未入力やメールアドレス形式のエラーを表示
- 登録内容をSQLiteデータベースへ保存
- 短時間に送信が集中した場合は、日本語のエラーを表示して受付を一時的に制限

### 管理者ログイン

- 管理者名とパスワードによるログイン
- ログイン試行が短時間に集中した場合は、日本語のエラーを表示して一時的に制限
...
| 分類 | 技術 |
| --- | --- |
| バックエンド | Python、Flask、Flask-Limiter |
| データベース | SQLite |
...
作成した `.env` に、次の5項目を自分の環境用として設定します。
...
| `APP_ENV` | ローカルでは `development`、HTTPSで公開する本番では `production` |
| `RATELIMIT_STORAGE_URI` | レート制限の記録先。ローカルでは `memory://`、本番では共有Redisの接続先 |
...
この起動方法はローカル開発用です。オンライン公開ではHTTPSと本番用WSGIサーバーまたはホスティングサービスを使い、Flaskの開発サーバーをそのまま公開しないでください。アプリのデバッグモードは無効にしています。

## オンライン公開前の安全設定

- 公開フォームは、HTMLの `maxlength` とAPI側の両方で、名前100文字、メールアドレス254文字、問い合わせ内容5,000文字の上限を確認します。
- リクエスト本文が64 KiBを超えた場合は、保存せず `413` エラーとして拒否します。
- 公開フォームの送信は接続元ごとに1分間10回、管理者ログインの試行は1分間5回までです。上限を超えると、画面に日本語の案内が表示されます。
- `X-Content-Type-Options`、`X-Frame-Options`、`Referrer-Policy`、`Content-Security-Policy` をすべての応答に付けます。現在のCSSとJavaScriptは同一サイト内のファイルだけを読み込む構成です。
- セッションCookieは常に `HttpOnly` と `SameSite=Lax` を使います。`APP_ENV=production` のときだけ `Secure` を付けるため、ローカルのHTTPログインを妨げず、本番ではHTTPS通信だけに送信されます。

### レート制限の記録先

ローカル開発では `.env.example` の `RATELIMIT_STORAGE_URI=memory://` をそのまま利用できます。記録はアプリのメモリ内だけにあり、再起動すると消え、複数プロセス間では共有されません。このため開発・テスト専用です。

本番では `APP_ENV=production` に変更し、`RATELIMIT_STORAGE_URI` に運用環境の共有Redis接続先を設定してください。Redisを使うと、複数プロセスでも同じ回数を参照できます。接続先にパスワードなどが含まれる場合は、実際の値を `.env.example`、README、Gitへ記録せず、公開先の秘密情報設定で管理してください。

## 自動テスト

公開フォーム、入力エラー、管理者認証、APIのアクセス制御、ログイン後の一覧取得・状態変更・削除に加え、入力上限、リクエスト容量、安全ヘッダー、Cookie設定、デバッグ設定、レート制限を確認する **29件の自動テスト** があります。
...
すべて成功すると、結果に `29 passed` と表示されます。

## 秘密情報をGitへ登録しないための注意

- `.env`、APIキー、秘密鍵、パスワード、実際のパスワードハッシュをGitへ登録しないでください。
- `.env.example` には安全な開発用の例または空欄だけを残し、実際の秘密情報を記載しないでください。
